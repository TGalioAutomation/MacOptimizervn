...
// MARK: - kiểu dữ liệu riêng tư

enum PrivacyType: String, CaseIterable, Identifiable {
    case history = "Lịch sử duyệt"
    case cookies = "Tệp cookie"
    case downloads = "Lịch sử tải xuống"
    case permissions = "Quyền ứng dụng"
    case recentItems = "Mục gần đây"
    case wifi = "Mạng Wi-Fi"
    case chat = "Thông tin trò chuyện"
    case development = "Dấu vết phát triển" // Terminal, VSCode, etc.
    ...
    var icon: String {
        ...
        case .permissions: return "lock.shield" // khóa quyền
        case .recentItems: return "clock" // dự án gần đây
        case .wifi: return "wifi" // Wi-Fi
        case .chat: return "message" // trò chuyện
        case .development: return "terminal" // phát triển
        }
    }
}

// ĐÁNH DẤU: - loại trình duyệt

enum BrowserType: String, CaseIterable, Identifiable {
    case safari = "Safari"
    case chrome = "Google Chrome"
    case firefox = "Firefox"
    case system = "System" // Mục hệ thống
...
struct PrivacyItem: Identifiable, Equatable {
    ...
    let path: URL
    let size: Int64
    let displayPath: String // Được sử dụng để hiển thị đường dẫn hoặc mô tả thân thiện hơn với người dùng
    var isSelected: Bool = true
    var children: [PrivacyItem]? = nil // Danh sách các mục con (dùng để mở rộng chi tiết)
}

// MARK: - Mẫu mục cấp phép ứng dụng
...
struct AppPermission: Identifiable, Equatable {
    ...
    let serviceName: String     // "Máy ảnh"
    let serviceCategory: String // "Riêng tư"
    let authValue: Int          // 0=từ chối, 2=cho phép
...
// MARK: - kiểu dữ liệu trình duyệt

enum BrowserDataType: String, CaseIterable, Identifiable {
    case browsingHistory = "Lịch sử duyệt web"
    case downloadHistory = "Lịch sử tải xuống"
    case cookies = "Cookies"
    case passwords = "Mật khẩu đã lưu"
    case autofillForms = "Biểu mẫu tự điền"
    case searchQueries = "Truy vấn tìm kiếm"
    case lastSession = "Phiên hoạt động gần nhất"
    case localStorage = "Lưu trữ cục bộ"
    case cache = "Bộ đệm"
...
// MARK: - Mô hình mục dữ liệu trình duyệt

struct BrowserDataItem: Identifiable, Equatable {
    let id = UUID()
    let browser: BrowserType
    let dataType: BrowserDataType
    let count: Int              // Số lượng mục: 377 cookie
    let size: Int64             // Kích thước tệp: 229 KB
    let profile: String         // Tên hồ sơ:Mặc định
    let appIcon: NSImage?       // Biểu tượng ứng dụng thực
    var isSelected: Bool = true
...
// MARK: - Dịch vụ quét bảo mật

class PrivacyScannerService: ObservableObject {
    @Published var privacyItems: [PrivacyItem] = []
    @Published var appPermissions: [AppPermission] = [] // Mới: Quyền ứng dụng được phân tích cú pháp
    @Published var browserDataItems: [BrowserDataItem] = [] // Mới: Dữ liệu trình duyệt được phân tích cú pháp
    ...
    func scanAll() async {
        ...
        // 6. Quét dấu vết phát triển (Terminal, VSCode)
    ...
    // MARK: - Phương thức trợ giúp: Thêm file liên quan (WAL/SHM)
    ...
    // MARK: - Phát hiện và chấm dứt quá trình
        ...
        for app in apps {
            ...
            if bundleId.contains("com.apple.Safari") || 
               ...
               bundleId.contains("org.mozilla.firefox") {
                ...
                // Đợi một lúc xem nó có đóng lại không
    ...
    /// Chuyển đổi đệ quy trạng thái đã chọn
        ...
        func toggle(in items: inout [PrivacyItem]) -> Bool {
            for i in 0..<items.count {
                if items[i].id == id {
                    ...
                    // Nếu có mục phụ, đồng bộ trạng thái đã chọn
                ...
                if items[i].children != nil {
                    if toggle(in: &items[i].children!) {
                        ...
                        // Cập nhật trạng thái cha mẹ (tùy chọn: Nếu tất cả con được chọn/hủy, cha mẹ cũng cần được cập nhật phải không? Logic phức tạp chưa được triển khai)
        ...
        for path in pathsToDelete {
            ...
            do {
                if fileManager.fileExists(atPath: path.path) {
                    ...
                    // ⚠️ Sửa lỗi bảo mật: Ưu tiên thùng rác

                    do {
                        try fileManager.trashItem(at: path, resultingItemURL: nil)
                        print("✅ [Clean] Moved to trash: \(path.lastPathComponent)")
                    } catch {
                        // Chỉ cố gắng xóa nó trực tiếp nếu thùng rác bị lỗi (dữ liệu riêng tư cần phải được xóa hoàn toàn)
                ...
                if error.code == 513 || error.domain == NSCocoaErrorDomain {
                    ...
                    print("   → Hãy cấp quyền \"Truy cập toàn bộ ổ đĩa\" trong Cài đặt hệ thống")
                } else {
    ...
    private func scanChrome() -> [PrivacyItem] {
        ...
        for profile in profiles {
            ...
            // ===== 1. Cơ sở dữ liệu Lịch sử phân tích =====
            ...
            // ===== 2. Phân tích cơ sở dữ liệu Cookie =====
            ...
            if fileManager.fileExists(atPath: cookiesURL.path) {
                ...
                if cookieCount > 0 {
                    // Chi tiết phân tích cú pháp (100 tên miền hàng đầu)
            ...
            // ===== 3. Phân tích dữ liệu đăng nhập (mật khẩu) =====
            ...
            // ===== 4. Phân tích dữ liệu web (tự động điền) =====
    ...
    private func scanFirefox() -> [PrivacyItem] {
        ...
        for profile in profiles {
            let profileName = profile.lastPathComponent
            if profileName.hasPrefix(".") { continue } // Bỏ qua các tập tin ẩn
            ...
            // 4. Cache
            // Firefox Cache thường nằm ở ~/Library/Caches/Firefox/Profiles/...
    ...
    private func scanPermissions() async -> [PrivacyItem] {
        ...
        // Phân tích cơ sở dữ liệu TCC để có được quyền ứng dụng chi tiết
        ...
        // 4. Cập nhật quyền ứng dụng

        await MainActor.run {
            self.appPermissions = allPermissions
        }
        
        // 5. Để tương thích: Đồng thời tạo các bản ghi quyền ở định dạng PrivacyItem (mỗi bản ghi cho mỗi quyền)
    ...
    private func fileSize(at url: URL) -> Int64? {
        // Có thể thất bại nếu không có quyền đọc
    ...
    // MARK: - Phân tích cơ sở dữ liệu TCC

    
    /// Phân tích cơ sở dữ liệu TCC và lấy danh sách cấp phép ứng dụng

    private func parseTCCDatabase(at url: URL) async -> [AppPermission] {
        var permissions: [AppPermission] = []
        
        // 1. Mở cơ sở dữ liệu SQLite
        ...
        // 2. Truy vấn tất cả các quyền (client_type = 0 có nghĩa là ID gói)
        ...
        while sqlite3_step(statement) == SQLITE_ROW {
            ...
            // Chỉ chứa các quyền được ủy quyền (authValue == 2 có nghĩa là cho phép)

            guard authValue == 2 else { continue }
            
            // 4. Nhận thông tin ứng tuyển
    ...
    /// Nhận biểu tượng và tên ứng dụng

    private func getAppInfo(bundleId: String) -> (String, NSImage)? {
        // 1. Hãy thử lấy nó từ NSWorkspace
        ...
        // 2. Cố gắng tìm từ những con đường chung
        ...
        // 3. Hạ cấp: Sử dụng Bundle ID nhưng lọc ra một số thành phần bên trong hệ thống

        if bundleId.contains("apple") && !bundleId.contains("com.apple.Safari") {
            return nil // Bỏ qua nội bộ của Apple
        }
